<nav class="hidden md:flex item-center justify-between text-xs text-gray-400">
    <ul class="flex uppercase font-semibold space-x-10 border-b-4 pb-3">
        <li><a wire:click.prevent="setStatus('All')" href="{{route('idea.index', ['status'=>'All'])}}" class="transition duration-150 ease-in border-b-4 pb-3 hover:border-blue @if(strcasecmp($status,'all')===0) border-blue text-gray-900 @endif">All Ideas ({{ array_sum($statusCount) ?? 0  }})</a></li>
